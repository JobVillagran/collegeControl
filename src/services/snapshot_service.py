...
class SnapshotService:
    ...
    def load_previous_snapshot(self) -> dict:
        return read_json(PREVIOUS_SNAPSHOT_FILE, default={"courses": [], "assignments": []})

    def save_current_snapshot(self, snapshot: dict) -> None:
        write_json(CURRENT_SNAPSHOT_FILE, snapshot)

    def rotate_snapshots(self, snapshot: dict) -> None:
        write_json(PREVIOUS_SNAPSHOT_FILE, snapshot)